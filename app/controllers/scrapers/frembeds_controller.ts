  ...
  scrapeMovies = async () => {
    ...
    for (let page = 1; page < totalPages; page++) {
      ...
      const newMoviesPromise = movies.map(async (movie) => {
        ...
        const collection = await tmdbService.getCollection(tmdbData.belongs_to_collection?.id)
        ...
        const mediaData = {
          ...
          backdropPath:
            tmdbData.backdrop_path && tmdbService.IMAGE_BASE_URL + tmdbData.backdrop_path,
          status: 'ENDED',
          type: 'MOVIE',
          trailerUrl,
          logoPath,
